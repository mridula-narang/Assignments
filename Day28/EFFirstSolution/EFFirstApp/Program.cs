...
namespace EFFirstApp
{
    internal class Program
    {
        ...
        {
            Supplier supplier = new Supplier { SupplierName = "ABC corp", Phone = "1234567890", Email = "abc@gmail.com" };
            context.Suppliers.Add(supplier);
            context.SaveChanges();
            Console.WriteLine("Supplier Added");
            Console.WriteLine($"Supplier id {supplier.SupplierId}");

            Category category = new Category {CategoryName = "Electronics",Description = "Electronic items"};
            context.Categories.Add(category);
            context.SaveChanges();
            Console.WriteLine("Category Added");
            Console.WriteLine($"Category id {category.CategoryId}");

            Product product = new Product { Name = "Laptop", Price = 50000, Quantity = 10, Image = "laptop.jpg", CategoryId = category.CategoryId };
            context.Products.Add(product);
            context.SaveChanges();
            Console.WriteLine("Product Added");
            Console.WriteLine($"Product id {product.Id}");

            SupplierProduct supplierProduct = new SupplierProduct { SupplierId = supplier.SupplierId, ProductId = product.Id };
